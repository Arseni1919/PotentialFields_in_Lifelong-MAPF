...
def find_span_roadmaps(the_agent, rest_of_agents, nodes_dict):
    ...
    if n_of_conflicts == 0:
        ...
        return to_replan_others, there_path_is_possible, [], None, None
...
def _plan_ways_aside(the_agent, k, h_func, nodes_dict, infected_agents, free_nodes_to_fill, directed_graph):
    ...
    while step_count < k - 1:
        ...
        for i_a in infected_agents:
            ...
            if last_node_name in node_names_of_plan:
                ...
                to_nodes_names_free.sort(key=lambda x: h_func(nodes_dict[x], last_node))
                nearest_out_free = to_nodes_names_free[0]

                next_to_move.sort(key=lambda x: h_func(nodes_dict[x], nodes_dict[nearest_out_free]))
                next_node_name = next_to_move[0]
                if next_node_name not in next_config:
                    i_a.plan.append(nodes_dict[next_node_name])
    ...
    # the_agent.dppibt_build_plan(h_agents=rest_of_agents)
...
class AlgSDS(AlgParObsPFPrPSeq):
    ...
    def _build_plans(self):
        ...
        # find free locations
        to_replan_o, there_is_path, inf_agents, free_nodes, dir_graph = self._find_free_spots_and_freeze_others()
    ...
    def _find_free_spots_and_freeze_others(self):
